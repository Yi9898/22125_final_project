...
# Do training
if ( ! -e out.$n ) then
    python $RDIR/ANN_train.py -nh 5 -syn syn.$n -t $DDIR/$a/fakeCV/f00$n -e $DDIR/$a/fakeCV/c00$n -stop | grep -v "#" > out.$n
endif

# Do evaluation
if ( ! -e c00$n.pred ) then
    python $RDIR/ANN_forward.py -syn syn.$n -e $DDIR/$a/fakeCV/c00$n | grep -v "#" > c00$n.pred
endif
