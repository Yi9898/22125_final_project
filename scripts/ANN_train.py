...
parser = argparse.ArgumentParser(description="A very first Python program")
#parser.add_argument("-h", action="store_true", dest="help", default=False, help="show this help message and exit")
parser.add_argument("-i", action="store", dest="EPOCHS", type=int, default= 100, help="Number of epochs to train (default 100)")
parser.add_argument("-t", action="store", dest="TRAINING_FILE", help="File with training data")
parser.add_argument("-e", action="store", dest="EVALUATION_FILE", help="File with evaluation data")
...
parser.add_argument("-syn", action="store", dest="SYNFILE_NAME", help="Name of synaps file")
parser.add_argument("-bl", action="store_true", default=False, dest='BL', help='Use Blosum encodingg')
parser.add_argument("-nh", action="store", dest="HIDDEN_LAYER_DIM", type=int, help="Number of hidden neurons)")
parser.add_argument("-stop", action="store_true", default=False, dest='early_stopping', help='Use early stopping')
...
def back_prop(X, t, w_h, w_o, dj_dw_h, dj_dw_o):
    ...
    for j in range(hidden_layer_dim):
        ...
        delta2 = delta * w_o[j][0] * (1 - X[1][j])*X[1][j] 
